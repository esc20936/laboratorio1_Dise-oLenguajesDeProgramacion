...
def Thompson(Regex):
    ...
    for char in Regex:
        if char == "|":
            if len(NFAstack) < 2:
                print(
                    "No hay suficientes NFA para realizar la operacion | (or)\nDescripcion: Deben existir simbolos de ambos lados del operador | (or)\n Formato esperado a|b \n Formato recibido:"
                )
                return (
                    {"Error": "No hay suficientes NFA para realizar la operacion |"},
                    None,
                )
            nfa2 = NFAstack.pop()
            nfa1 = NFAstack.pop()
            ...
            if len(NFAstack) < 1:
                print(
                    "Error: No hay suficientes NFA para realizar la operacion * (cerradura de kleene)\n Formato esperado a* \n Formato recibido:"
                )
                return (
                    {"Error": "No hay suficientes NFA para realizar la operacion *"},
                    None,
            ...
            if len(NFAstack) < 1:
                print(
                    "Error: No hay suficientes NFA para realizar la operacion + (cerradura positiva)\n Formato esperado a+ \n Formato recibido:"
                )
                return (
                    {"Error": "No hay suficientes NFA para realizar la operacion +"},
                    None,
            ...
            if len(NFAstack) < 1:
                print(
                    "Error: No hay suficientes NFA para realizar la operacion ? (opcional)\n Formato esperado a? \n Formato recibido:"
                )
                return (
                    {"Error": "No hay suficientes NFA para realizar la operacion ?"},
                    None,
