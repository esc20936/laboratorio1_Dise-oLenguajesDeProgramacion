...
def minimize_dfa(transition_table, acceptance_states):
    ...
    while len(reachable_states) != old_size:
        ...
        for state in reachable_states.copy():
            for symbol in transition_table.columns[1:]:
                next_state = transition_table.loc[
                    transition_table["Estados"] == state, symbol
                ].values[0]
                reachable_states.add(next_state)
    reachable_states = list(reachable_states)
    reachable_table = transition_table.loc[
        transition_table["Estados"].isin(reachable_states)
    ...
    table = pd.DataFrame(
        index=range(len(partition)), columns=transition_table.columns[1:], dtype=object
    )
    ...
    while True:
        ...
        for i, group in enumerate(partition):
            for symbol in transition_table.columns[1:]:
                partition_dict = {}
                for state in group:
                    next_state = reachable_table.loc[
                        reachable_table["Estados"] == state, symbol
                    ].values[0]
                    for j, subgroup in enumerate(partition):
                        if next_state in subgroup:
                            partition_dict[j] = partition_dict.get(j, set()) | set(
                                [state]
                            )
    ...
    state_map = dict(
        sum(
            [
                [(state, new_states[i]) for state in group]
                for i, group in enumerate(partition)
            ],
            [],
        )
