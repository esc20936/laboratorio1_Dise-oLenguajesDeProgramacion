...
class FA(object):
    ...
    def print_automata(
        self, type, i_state, t_state, states, symbols, t_function, state_mapping=None
    ):
        print(
            Colors.OKBLUE
            + Colors.ENDC
...
class DFA(FA):
        ...
        while len(dstates_u) > 0:
            ...
            for symbol in self.symbols:
                ...
                if len(U) > 0:
                    ...
                    t_func[(subset_mapping[tuple(T)], symbol)] = subset_mapping[
                        tuple(U)
                    ]
    ...
    def subset(self):
        ...
        dstates_u = [
            self.e_closure_state(self.nfa.initial_state, self.nfa.transition_function)
        ]
        dstates_m = []
        ...
        while len(dstates_u) > 0:
            ...
            for symbol in self.symbols:
                U = self.e_closure_set(
                    self.move(T, symbol, self.nfa.transition_function),
